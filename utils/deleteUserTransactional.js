...
async function expireEmployerJobsAndRejectInterests(employerId, deletedAt, transaction) {
  if (!Number.isFinite(Number(employerId)) || Number(employerId) <= 0) return;
...
async function rejectEmployeeSentInterests(employeeId, transaction) {
  if (!Number.isFinite(Number(employeeId)) || Number(employeeId) <= 0) return;
...
async function snapshotAndDeleteUser({
  ...
  if (employee) {
    await rejectEmployeeSentInterests(employee.id, transaction);
  ...
  if (employer) {
    await expireEmployerJobsAndRejectInterests(employer.id, deletedAt, transaction);
