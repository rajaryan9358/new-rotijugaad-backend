...
router.get('/:id', async (req, res) => {
  try {
    ...
    let reporter_entity = null;
    let reported_entity = null;
    let reason = null;

    try {
      if (reportType === 'job') {
        if (Employee && row.user_id) {
          reporter_entity = await Employee.findByPk(row.user_id, { paranoid: true });
        }
        if (Job && row.report_id) {
          const job = await Job.findByPk(row.report_id, { paranoid: true });
          if (job) {
            const plain = job.get ? job.get({ plain: true }) : { ...job };
            if (plain.job_profile_id && JobProfile) {
              const jp = await JobProfile.findByPk(plain.job_profile_id, { paranoid: true });
              if (jp) plain.JobProfile = jp.get ? jp.get({ plain: true }) : jp;
            }
            reported_entity = plain;
          }
        }
        if (EmployeeReportReason && row.reason_id) {
          reason = await EmployeeReportReason.findByPk(row.reason_id, { paranoid: true });
        }
      } else if (reportType === 'employee') {
        if (Employer && row.user_id) {
          reporter_entity = await Employer.findByPk(row.user_id, { paranoid: true });
        }
        if (Employee && row.report_id) {
          reported_entity = await Employee.findByPk(row.report_id, { paranoid: true });
        }
        if (EmployerReportReason && row.reason_id) {
          reason = await EmployerReportReason.findByPk(row.reason_id, { paranoid: true });
        }
      }
    } catch (e) {
      console.error('[reports] detail enrichment error', e);
    }

    row.setDataValue('reporter_entity', reporter_entity);
    row.setDataValue('reported_entity', reported_entity);
    row.setDataValue('reason', reason);
