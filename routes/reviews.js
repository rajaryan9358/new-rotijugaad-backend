...
const hydrateReviewerNames = async (rows) => {
  const employeeUserIds = rows.filter((r) => r.user_type === 'employee').map((r) => r.user_id);
  const employerUserIds = rows.filter((r) => r.user_type === 'employer').map((r) => r.user_id);

  const unique = (arr) => [...new Set(arr)];
  const employees = employeeUserIds.length
    ? await Employee.findAll({ where: { user_id: unique(employeeUserIds) }, attributes: ['id', 'user_id', 'name', 'name_hindi'] })
    : [];
  const employers = employerUserIds.length
    ? await Employer.findAll({ where: { user_id: unique(employerUserIds) }, attributes: ['id', 'user_id', 'name', 'name_hindi', 'organization_name'] })
    : [];

  // keyed by users.id (the FK stored in review.user_id)
  ...
  return rows.map((row) => {
    ...
    const reviewer_name = entity ? deriveName(entity) : '-';
    const entityId = entity?.id ?? null;

    return {
      ...json,
      reviewer_name,
      reviewer_entity_id: entityId,
      reviewer_route: entityId
        ? (row.user_type === 'employee' ? `/employees/${entityId}` : `/employers/${entityId}`)
        : null,
    };
  });
};
    ...
    if (!review.read_at) {
      ...
      const reviewer =
        review.user_type === 'employee'
          ? await Employee.findOne({ where: { user_id: review.user_id }, attributes: ['id', 'name'], paranoid: false })
          : await Employer.findOne({ where: { user_id: review.user_id }, attributes: ['id', 'name'], paranoid: false });
