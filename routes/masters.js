...
const { getAllMasters } = require('../apis/masters/masters.controller');
...
/**
 * GET /
 * Mobile: fetch all master data in one call.
 */
 ...
 * List all additional document types ordered by sequence.
 */
router.get('/additional-document-types', async (req, res) => {
  try {
    const types = await AdditionalDocumentType.findAll({ order: [['sequence', 'ASC']] });
    res.json({ success: true, data: types });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
...
/**
 * GET /additional-document-types/:id
 * Retrieve a single additional document type by id.
 */
router.get('/additional-document-types/:id', async (req, res) => {
  try {
    const type = await AdditionalDocumentType.findByPk(req.params.id);
    if (!type) return res.status(404).json({ success: false, message: 'Additional Document Type not found' });
    res.json({ success: true, data: type });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
...
/**
 * POST /additional-document-types
 * Create a new additional document type.
 */
router.post('/additional-document-types', async (req, res) => {
  try {
    const { type_english, type_hindi, sequence, is_active } = req.body;
    if (!type_english || !type_hindi) {
      return res.status(400).json({ success: false, message: 'English and Hindi names are required' });
    }
    const type = await AdditionalDocumentType.create({
      type_english,
      type_hindi,
    ...
    await safeCreateLog({
      ...
      type: 'add',
      redirect_to: '/masters/additional-document-types',
      log_text: `Additional Document Type added: ${type.type_english} (${type.type_hindi})`,
      rj_employee_id: getAdminId(req),
    });
...
/**
 * PUT /additional-document-types/:id
 * Update an existing additional document type.
 */
router.put('/additional-document-types/:id', async (req, res) => {
  try {
    const type = await AdditionalDocumentType.findByPk(req.params.id);
    if (!type) return res.status(404).json({ success: false, message: 'Additional Document Type not found' });
    const { type_english, type_hindi, sequence, is_active } = req.body;
    ...
    await safeCreateLog({
      ...
      type: 'update',
      redirect_to: '/masters/additional-document-types',
      log_text: `Additional Document Type updated: ${beforeTypeEnglish} (${beforeTypeHindi}) → ${type.type_english} (${type.type_hindi}) (active ${beforeIsActive} → ${type.is_active})`,
      rj_employee_id: getAdminId(req),
    });
...
/**
 * DELETE /additional-document-types/:id
 * Permanently delete an additional document type.
 */
router.delete('/additional-document-types/:id', async (req, res) => {
  try {
    const type = await AdditionalDocumentType.findByPk(req.params.id);
    if (!type) return res.status(404).json({ success: false, message: 'Additional Document Type not found' });
    const deletedTypeEnglish = type.type_english;
    ...
    await safeCreateLog({
      ...
      type: 'delete',
      redirect_to: '/masters/additional-document-types',
      log_text: `Additional Document Type deleted: ${deletedTypeEnglish} (${deletedTypeHindi})`,
      rj_employee_id: getAdminId(req),
    });

    res.json({ success: true, message: 'Additional Document Type deleted successfully' });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
});
...
/**
 * PUT /additional-document-types/bulk/sequence
 * Update sequence values for multiple additional document types.
 */
router.put('/additional-document-types/bulk/sequence', async (req, res) => {
  try {
    const { types } = req.body;
    if (!Array.isArray(types)) return res.status(400).json({ success: false, message: 'Types array required' });
    await Promise.all(types.map((type) => AdditionalDocumentType.update({ sequence: type.sequence }, { where: { id: type.id } })));
    res.json({ success: true, message: 'Sequences updated successfully' });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
...
router.post('/volunteers', async (req, res) => {
  try {
    const namePair = await fillBilingualPair(req.body?.name, req.body?.name_hindi);
    const addressPair = await fillBilingualPair(req.body?.address, req.body?.address_hindi);
    const descriptionPair = await fillBilingualPair(req.body?.description, req.body?.description_hindi);

    const payload = {
      name: (namePair.english || '').toString().trim(),
      name_hindi: namePair.hindi || null,
      phone_number: (req.body?.phone_number || '').toString().trim(),
      assistant_code: (req.body?.assistant_code || '').toString().trim() || null,
      address: addressPair.english || null,
      address_hindi: addressPair.hindi || null,
      description: descriptionPair.english || null,
      description_hindi: descriptionPair.hindi || null,
    };
...
router.put('/volunteers/:id', async (req, res) => {
  try {
    ...
    if (payload.name_hindi !== undefined) payload.name_hindi = String(payload.name_hindi || '').trim() || null;
    ...
    if (payload.address_hindi !== undefined) payload.address_hindi = String(payload.address_hindi || '').trim() || null;
    if (payload.description !== undefined) payload.description = String(payload.description || '').trim() || null;
    if (payload.description_hindi !== undefined) payload.description_hindi = String(payload.description_hindi || '').trim() || null;

    const wantsName = Object.prototype.hasOwnProperty.call(payload, 'name')
      || Object.prototype.hasOwnProperty.call(payload, 'name_hindi');
    if (wantsName) {
      const pair = await fillBilingualPair(payload.name, payload.name_hindi);
      payload.name = pair.english;
      payload.name_hindi = pair.hindi;
    }

    const wantsAddress = Object.prototype.hasOwnProperty.call(payload, 'address')
      || Object.prototype.hasOwnProperty.call(payload, 'address_hindi');
    if (wantsAddress) {
      const pair = await fillBilingualPair(payload.address, payload.address_hindi);
      payload.address = pair.english;
      payload.address_hindi = pair.hindi;
    }

    const wantsDescription = Object.prototype.hasOwnProperty.call(payload, 'description')
      || Object.prototype.hasOwnProperty.call(payload, 'description_hindi');
    if (wantsDescription) {
      const pair = await fillBilingualPair(payload.description, payload.description_hindi);
      payload.description = pair.english;
      payload.description_hindi = pair.hindi;
    }
