...
router.post('/', async (req, res) => {
  try {
    ...
    const targetType = String(s.user_type || '').trim().toLowerCase();
    if (targetType === 'employee' || targetType === 'employer') {
      ...
      await notifyUsers({
        users,
        titleEnglish: 'New Story',
        titleHindi: 'नई स्टोरी',
        bodyEnglish: s.title_english || 'A new story has been added.',
        bodyHindi: s.title_hindi || s.title_english || 'नई स्टोरी जोड़ी गई है।',
        referenceType: 'story',
        referenceId: s.id,
        persist: false,
        extraData: {
