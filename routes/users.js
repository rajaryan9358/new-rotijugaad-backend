...
async function ensureUserEntityForType(userInstance, options = {}) {
  ...
  if (normalizedType === 'employee') {
    ...
    if (existing) {
      if (existing.deleted_at && typeof existing.restore === 'function') {
...
router.get('/:id', async (req, res) => {
  try {
    ...
    if (!user) return res.status(404).json({ success: false, message: 'User not found' });
    await ensureUserEntityForType(user);
  ...
  try {
    ...
    if (existing) {
      if (existing.deleted_at) {
        ...
        await ensureUserEntityForType(existing);
    ...
    const nameRaw = name == null ? '' : String(name).trim();
    ...
    const namePair = nameRaw
      ? await fillBilingualPair(nameRaw, nameHindi)
      : { english: null, hindi: null };

    const user = await User.create({
      mobile,
      name: namePair.english || null,
