...
const { notifyEmployeeStatusChange } = require('../utils/systemNotifications');
...
const buildEmployeeCreditMessage = ({ contactDelta, interestDelta }) => {
  const partsEn = [];
  const partsHi = [];
...
const pickEmployeePayload = (body = {}) => ({
  ...
  name_hindi: (body.name_hindi ?? null) === '' ? null : body.name_hindi,
  dob: body.dob || null,
  gender: body.gender || null,
  state_id: normalizeIntOrNull(body.state_id),
  city_id: normalizeIntOrNull(body.city_id),
  lat: normalizeDecimalOrNull(body.lat ?? body.latitude),
  lng: normalizeDecimalOrNull(body.lng ?? body.longitude),
  preferred_state_id: normalizeIntOrNull(body.preferred_state_id),
  preferred_city_id: normalizeIntOrNull(body.preferred_city_id),
  preferred_location: (body.preferred_location ?? null) === '' ? null : (body.preferred_location ?? null),
  ...
  about_user_hindi: body.about_user_hindi || null,
...
const validateEmployeeCreditState = (currentEmployee, nextPayload = {}) => {
...
router.get('/', async (req, res) => {
  try {
    ...
    const skillFilter = normalizeIntOrNull(req.query.skillFilter);
    ...
    if (Object.keys(userWhere).length || Object.getOwnPropertySymbols(userWhere).length) {
    ...
    // skill filter via employee_skills
    if (skillFilter) {
      const skillRows = await EmployeeSkill.findAll({
        attributes: ['user_id'],
        where: { skill_id: skillFilter }
      });
      const skillEmployeeIds = [...new Set(skillRows.map(r => r.user_id).filter(Boolean))];
      if (!skillEmployeeIds.length) {
        return res.json({ success: true, data: [], meta: { page, pageSize, total: 0, totalPages: 1 } });
      }
      where.id = { ...(where.id || {}), [Op.in]: skillEmployeeIds };
    }
    ...
    // Build job_profiles_display + work_natures_display + skills_display for list UI.
    ...
    if (employeeIds.length) {
      ...
      const skillRows = await EmployeeSkill.findAll({
        where: { user_id: employeeIds },
        attributes: ['user_id', 'skill_id']
      });
      if (skillRows.length) {
        const allSkillIds = [...new Set(skillRows.map(r => r.skill_id).filter(Boolean))];
        const skillRecords = await Skill.findAll({ where: { id: allSkillIds }, attributes: ['id', 'skill_english'] });
        const skillById = new Map(skillRecords.map(s => [s.id, s.skill_english]));
        for (const r of skillRows) {
          const list = skillsMap.get(r.user_id) || [];
          const label = skillById.get(r.skill_id);
          if (label) list.push(label);
          skillsMap.set(r.user_id, list);
        }
      }
    }
...
router.get('/:id', async (req, res) => {
  try {
    ...
    const empSkillRows = await EmployeeSkill.findAll({
      where: { user_id: employeeId },
      attributes: ['skill_id']
    });
    const empSkillIds = empSkillRows.map(r => r.skill_id).filter(Boolean);
    const skillRecords = empSkillIds.length
      ? await Skill.findAll({ where: { id: empSkillIds }, attributes: ['id', 'skill_english'] })
      : [];
    const skills = skillRecords.map(r => r.skill_english).filter(Boolean);
    ...
    const data = {
      ...
      skills_display: skills.length ? [...new Set(skills)].join(', ') : null
    };
...
router.post('/', authenticate, async (req, res) => {
  try {
    ...
    const nameHindi = (req.body?.name_hindi ?? null) === '' ? null : req.body?.name_hindi;
    ...
    const aboutPair = await fillBilingualPair(req.body?.about_user, req.body?.about_user_hindi);
    ...
    await sequelize.transaction(async (t) => {
      ...
      if (userId) {
        ...
        if (namePair.english && (!user.name || !user.name.toString().trim())) userUpdates.name = namePair.english;
        if (namePair.hindi && (!user.name_hindi || !user.name_hindi.toString().trim())) userUpdates.name_hindi = namePair.hindi;
        ...
        if (user) {
          ...
          user = await User.create({ mobile, name: namePair.english, name_hindi: namePair.hindi, user_type: 'employee' }, { transaction: t });
          createdUser = user;
        }
      }
      ...
      empPayload.name = namePair.english;
      empPayload.name_hindi = namePair.hindi;
      if (aboutPair.english !== null || aboutPair.hindi !== null) {
        empPayload.about_user = aboutPair.english;
        empPayload.about_user_hindi = aboutPair.hindi;
      }
      if (!empPayload.verification_status) empPayload.verification_status = 'pending';
...
router.put('/:id', authenticate, async (req, res) => {
  try {
    ...
    await sequelize.transaction(async (t) => {
      ...
      beforeVerificationStatus = String(employee.verification_status || '').trim().toLowerCase();
      beforeKycStatus = String(employee.kyc_status || '').trim().toLowerCase();
      // Stamp when the employee/employer profile is created (used to prevent re-using the same mobile once a profile exists)
      if (user && !user.profile_completed_at) {
        await user.update({ profile_completed_at: new Date() }, { transaction: t });
      }
      ...
      if (wantsName) {
        await applyBilingualPairFromBody({ body: req.body || {}, payload: empPayload, englishKey: 'name', hindiKey: 'name_hindi', existing: employee });
      }

      const wantsAbout = Object.prototype.hasOwnProperty.call(req.body || {}, 'about_user')
        || Object.prototype.hasOwnProperty.call(req.body || {}, 'about_user_hindi');
      if (wantsAbout) {
        await applyBilingualPairFromBody({ body: req.body || {}, payload: empPayload, englishKey: 'about_user', hindiKey: 'about_user_hindi', existing: employee });
      }

      const creditValidationError = validateEmployeeCreditState(employee, empPayload);
      if (creditValidationError) {
        const err = new Error(creditValidationError);
        err.status = 400;
        throw err;
      }
      ...
      const mobile = (req.body?.mobile || '').toString().trim();
      const name = (empPayload?.name || '').toString().trim();
      if (user) {
        const userUpdates = {};
        ...
        if (
          Object.prototype.hasOwnProperty.call(empPayload, 'name_hindi') &&
          empPayload.name_hindi !== user.name_hindi
        ) {
          userUpdates.name_hindi = empPayload.name_hindi;
        }

        const verificationWasChanged = (
          Object.prototype.hasOwnProperty.call(empPayload, 'verification_status')
          && String(empPayload.verification_status || '').trim().toLowerCase() !== String(employee.verification_status || '').trim().toLowerCase()
        );

        if (verificationWasChanged && adminId) {
          await employee.update({ status_changed_by: adminId }, { transaction: t });
        }
    ...
    const nextVerificationStatus = String(updatedEmployee?.verification_status || '').trim().toLowerCase();
    const nextKycStatus = String(updatedEmployee?.kyc_status || '').trim().toLowerCase();

    if (
      beforeVerificationStatus !== nextVerificationStatus
      && (nextVerificationStatus === 'verified' || nextVerificationStatus === 'rejected')
    ) {
      notifyEmployeeStatusChange({
        employeeId: updatedEmployee.id,
        field: 'verification_status',
        nextValue: nextVerificationStatus,
      }).catch((err) => {
        console.error('[employees:update] notifyEmployeeStatusChange verification failed', err);
      });
    }

    if (
      beforeKycStatus !== nextKycStatus
      && (nextKycStatus === 'verified' || nextKycStatus === 'rejected')
    ) {
      notifyEmployeeStatusChange({
        employeeId: updatedEmployee.id,
        field: 'kyc_status',
        nextValue: nextKycStatus,
      }).catch((err) => {
        console.error('[employees:update] notifyEmployeeStatusChange kyc failed', err);
      });
    }
...
const handleEmployeeStatusMutation = async (req, res, field, value, message, extraPayload = null) => {
  try {
    const employee = await Employee.findByPk(req.params.id, { paranoid: false });
    if (!employee) return res.status(404).json({ success: false, message: 'Employee not found' });

    const prevValue = String(employee?.[field] || '').trim().toLowerCase();
    ...
    if (prevValue !== nextValue && (nextValue === 'verified' || nextValue === 'rejected')) {
      ...
      notifyEmployeeStatusChange({
        ...
        console.error('[employees] notifyEmployeeStatusChange failed', err);
      });
    }
...
router.post('/:id/add-credits', authenticate, async (req, res) => {
  try {
    ...
    const now = new Date();
    const currentExpiry = employee.credit_expiry_at ? new Date(employee.credit_expiry_at) : null;
    ...
    const updatePayload = {
      total_contact_credit: hasActiveCredits
        ? Number(employee.total_contact_credit || 0) + Math.max(contactDelta, 0)
        : Math.max(contactDelta, 0),
      contact_credit: hasActiveCredits
        ? Number(employee.contact_credit || 0) + Math.max(contactDelta, 0)
        : Math.max(contactDelta, 0),
      total_interest_credit: hasActiveCredits
        ? Number(employee.total_interest_credit || 0) + Math.max(interestDelta, 0)
        : Math.max(interestDelta, 0),
      interest_credit: hasActiveCredits
        ? Number(employee.interest_credit || 0) + Math.max(interestDelta, 0)
        : Math.max(interestDelta, 0),
    };
    ...
    await safeLog(req, {
      ...
      type: 'update',
      redirect_to: employeeRedirect(employee.id),
      log_text: `Employee credits added: #${employee.id} +${Math.max(contactDelta, 0)} contact, +${Math.max(interestDelta, 0)} interest, reason=${reason}`,
    });
    ...
    await safeLog(req, {
      ...
      type: 'update',
      redirect_to: employeeRedirect(employee.id),
      log_text: `Employee credits updated (employee log): #${employee.id} +${Math.max(contactDelta, 0)} contact, +${Math.max(interestDelta, 0)} interest, reason=${reason}`,
    });

    const linkedUser = await User.findByPk(employee.user_id, {
      paranoid: false,
    ...
    return res.json({
      ...
      data: {
        total_contact_credit: employee.total_contact_credit,
        contact_credit: employee.contact_credit,
        total_interest_credit: employee.total_interest_credit,
        interest_credit: employee.interest_credit,
...
router.get('/:id/documents', async (req, res) => {
  try {
    ...
    const payload = rows.map((row) => (row.toJSON ? row.toJSON() : row));
    const typeIds = Array.from(new Set(payload.map((row) => Number(row.document_type)).filter((v) => Number.isInteger(v) && v > 0)));
    const additionalTypes = typeIds.length
      ? await AdditionalDocumentType.findAll({
        where: { id: typeIds },
        attributes: ['id', 'type_english', 'type_hindi', 'sequence'],
      })
      : [];
    const typeMap = new Map(additionalTypes.map((row) => {
      const json = row.toJSON ? row.toJSON() : row;
      return [json.id, json];
    }));

    payload.forEach((row) => {
      const typeId = Number(row.document_type);
      row.DocumentType = Number.isInteger(typeId) ? (typeMap.get(typeId) || null) : null;
      row.AdditionalDocumentType = row.DocumentType;
    });
 ...
 */
router.post('/:id/documents/upload', docUpload.fields([{ name: 'file', maxCount: 1 }, { name: 'document', maxCount: 1 }, { name: 'document_file', maxCount: 1 }]), async (req, res) => {
  try {
    const emp = await Employee.findByPk(req.params.id);
    if (!emp) return res.status(404).json({ success:false, message:'Employee not found' });

    const file = (req.files?.file?.[0] || req.files?.document?.[0] || req.files?.document_file?.[0]) || null;
    if (!file) return res.status(400).json({ success:false, message:'No file uploaded' });

    const requestedTypeId = Number(req.body?.document_type_id || req.body?.document_type || 0);
    const legacyType = (req.query.type || '').toLowerCase().trim();
    let documentTypeValue = null;
    let documentTypeLabel = '-';

    if (Number.isInteger(requestedTypeId) && requestedTypeId > 0) {
      const additionalType = await AdditionalDocumentType.findByPk(requestedTypeId, {
        attributes: ['id', 'type_english', 'type_hindi', 'is_active'],
      });
      if (!additionalType || additionalType.is_active === false) {
        return res.status(400).json({ success:false, message:'Invalid additional document type' });
      }
      documentTypeValue = String(additionalType.id);
      documentTypeLabel = additionalType.type_english || additionalType.type_hindi || `Type #${additionalType.id}`;
    } else if (['resume','driving_license','other'].includes(legacyType)) {
      documentTypeValue = legacyType;
      documentTypeLabel = legacyType === 'resume' ? 'Resume' : legacyType === 'driving_license' ? 'Driving License' : 'Other';
    }
    ...
    const responseDoc = doc.toJSON ? doc.toJSON() : doc;
    if (Number.isInteger(requestedTypeId) && requestedTypeId > 0) {
      const additionalType = await AdditionalDocumentType.findByPk(requestedTypeId, {
        attributes: ['id', 'type_english', 'type_hindi', 'sequence'],
      });
      responseDoc.DocumentType = additionalType ? (additionalType.toJSON ? additionalType.toJSON() : additionalType) : null;
      responseDoc.AdditionalDocumentType = responseDoc.DocumentType;
    }
    ...
    await safeLog(req, {
      ...
      log_text: `Employee document uploaded: #${emp.id} ${emp.name || "-"} type=${documentTypeLabel} title=${file?.originalname || "-"}`
    });
    ...
    await safeLog(req, {
      ...
      type: 'update',
      redirect_to: employeeRedirect(emp.id),
      log_text: `Employee document updated (employee log): #${emp.id} ${emp.name || "-"} type=${documentTypeLabel} title=${file?.originalname || "-"}`
    });
 ...
 * Fetch skills linked to an employee.
 */
router.get('/:id/skills', async (req, res) => {
  try {
    const employeeId = parseInt(req.params.id, 10);
    if (!employeeId) return res.status(400).json({ success: false, message: 'Invalid employee id' });
    const esRows = await EmployeeSkill.findAll({
      where: { user_id: employeeId },
      attributes: ['id', 'user_id', 'skill_id']
    });
    const skillIds = esRows.map(r => r.skill_id).filter(Boolean);
    const skillRecords = skillIds.length
      ? await Skill.findAll({ where: { id: skillIds }, attributes: ['id', 'skill_english', 'skill_hindi', 'is_active'] })
      : [];
    const skillById = new Map(skillRecords.map(s => [s.id, s.toJSON ? s.toJSON() : s]));
    const data = esRows.map(r => ({ ...r.toJSON(), Skill: skillById.get(r.skill_id) || null }));
    res.json({ success: true, data });
  } catch (err) {
    console.error('[employees/:id/skills] get error:', err);
    res.status(500).json({ success: false, message: err.message });
  }
});
 ...
 * Replace employee skills (destroy all + bulkCreate).
 */
router.post('/:id/skills', authenticate, async (req, res) => {
  try {
    ...
    const incoming = req.body.skill_ids;
    if (!Array.isArray(incoming)) return res.status(400).json({ success: false, message: 'skill_ids must be an array' });

    const ids = [...new Set(incoming.map(n => parseInt(n, 10)).filter(n => !Number.isNaN(n) && n > 0))];

    await EmployeeSkill.destroy({ where: { user_id: employeeId } });

    if (ids.length) {
      const foundSkills = await Skill.findAll({ where: { id: ids }, attributes: ['id'] });
      const validIds = foundSkills.map(s => s.id);
      if (validIds.length) {
        await EmployeeSkill.bulkCreate(validIds.map(skill_id => ({ user_id: employeeId, skill_id })));
      }
    }
    ...
    const savedSkillIds = esRows.map(r => r.skill_id).filter(Boolean);
    const savedSkillRecords = savedSkillIds.length
      ? await Skill.findAll({ where: { id: savedSkillIds }, attributes: ['id', 'skill_english', 'skill_hindi', 'is_active'] })
      : [];
    const skillById = new Map(savedSkillRecords.map(s => [s.id, s.toJSON ? s.toJSON() : s]));
    const data = esRows.map(r => ({ ...r.toJSON(), Skill: skillById.get(r.skill_id) || null }));
    ...
    await safeLog(req, {
      ...
      type: 'update',
      redirect_to: employeeRedirect(employeeId),
      log_text: `Employee skills updated: #${employeeId} -> [${ids.join(', ')}]`
    });

    res.json({ success: true, data });
  } catch (err) {
    console.error('[employees/:id/skills] post error:', err);
    res.status(500).json({ success: false, message: err.message });
  }
});
 ...
 */
router.post('/experiences/upload/certificate', certificateUpload.fields([{ name: 'certificate', maxCount: 1 }, { name: 'experience_certificate', maxCount: 1 }, { name: 'file', maxCount: 1 }]), (req, res) => {
  try {
    const file = (req.files?.certificate?.[0] || req.files?.experience_certificate?.[0] || req.files?.file?.[0]) || null;
    if (!file) {
...
/**
 * GET /employees/:id/contacts-unlocked
 * Show contacts unlocked by the employee via employee_contacts.
 */
router.get('/:id/contacts-unlocked', async (req, res) => {
  try {
    ...
    const employee = await Employee.findByPk(employeeId, { attributes: ['id'], paranoid: false });
    if (!employee) return res.status(404).json({ success: false, message: 'Employee not found' });
    ...
    const employers = employerIds.length
      ...
      : [];
    const employerMap = Object.fromEntries(employers.map((row) => [row.id, row]));
    ...
    const jobs = jobIds.length
      ...
      : [];
    const jobMap = Object.fromEntries(jobs.map((row) => [row.id, row]));

    const data = contacts.map((contact) => {
      const employer = employerMap[contact.employer_id];
      const job = jobMap[contact.job_id];
      const orgType = (employer?.organization_type || '').toString().trim().toLowerCase();
      const organizationName = (orgType === 'domestic' || orgType === 'household')
        ? 'Household'
    ...
    console.error('[employees/:id/contacts-unlocked] error:', err);
    return res.status(500).json({ success: false, message: err.message });
  }
});
