...
router.get('/employee-plans/:id', async (req, res) => {
  try {
    const plan = await findByPkWithOptionalAttribute(EmployeeSubscriptionPlan, req.params.id);
...
router.post('/employee-plans', async (req, res) => {
  try {
    ...
    const plan = await createWithOptionalAttribute(EmployeeSubscriptionPlan, {
      plan_name_english,
      ...
      discounted_price: discounted_price === undefined || discounted_price === null || discounted_price === '' ? null : Number(discounted_price),
...
router.put('/employee-plans/:id', async (req, res) => {
  try {
    ...
    if (payload.discounted_price === '') payload.discounted_price = null;
    if (payload.discounted_price !== undefined && payload.discounted_price !== null) payload.discounted_price = Number(payload.discounted_price);
    ...
    await updateInstanceWithOptionalAttribute(plan, payload);
...
router.get('/employer-plans/:id', async (req, res) => {
  try {
    const plan = await findByPkWithOptionalAttribute(EmployerSubscriptionPlan, req.params.id);
    if (!plan) return res.status(404).json({ success: false, message: 'Plan not found' });
    res.json({ success: true, data: plan });
  } catch (error) {
...
router.post('/employer-plans', async (req, res) => {
  try {
    ...
    const plan = await createWithOptionalAttribute(EmployerSubscriptionPlan, {
      plan_name_english,
      plan_name_hindi: plan_name_hindi || null,
      plan_validity_days: Number(plan_validity_days) || 0,
      plan_tagline_english: plan_tagline_english || null,
