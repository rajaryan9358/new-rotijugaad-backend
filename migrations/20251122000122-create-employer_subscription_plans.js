...
module.exports = {
  async up(q,S){
    let c; try{ c=await q.describeTable('employer_subscription_plans'); }catch{}
    if(!c){
      await q.createTable('employer_subscription_plans',{
        ...
        ad_credits:{ type:S.INTEGER, allowNull:false, defaultValue:0 },
  ...
  async down(q){ try{ await q.dropTable('employer_subscription_plans'); }catch{} }
};
