...
module.exports = {
  async up(queryInterface, Sequelize) {
    const tableExists = await queryInterface.showAllTables().then(tables => tables.includes('employee_subscription_plans'));
    if (!tableExists) {
      await queryInterface.createTable('employee_subscription_plans', {
  ...
  async down(queryInterface) {
    await queryInterface.dropTable('employee_subscription_plans');
  }
};
